...
| S5 | Phase 4 tests + remote verification | Done | local full suite `pytest -q` => `322 passed`; host 208: services active, socket `660 999:986`, direct shell calls denied (`Permission denied` / `peer_unit_not_allowed`) | no new blocker; residual risk is deployment misconfiguration of unit names |
